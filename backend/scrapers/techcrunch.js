...
async function scrapeTechCrunch() {
  ...
  // Dedupe & DB-check
  const seen = new Set();
  const uniq = previews.filter(a => !seen.has(a.url) && seen.add(a.url));
  const existing = await Article.find({ url: { $in: uniq.map(a => a.url) } }).select('url');
  const dbSet = new Set(existing.map(a => a.url));
  const newArticles = uniq.filter(a => !dbSet.has(a.url));

  console.log(`🔍 Found ${uniq.length} unique previews, ${newArticles.length} new articles to process`);

  for (const art of newArticles) {
    try {
        
         const isNew = await redisClient.sadd('scraped:tech:urls', art.url);
      if (isNew === 0) continue;           // already seen before

      // Set a TTL on the set key (once)
      if ((await redisClient.ttl('scraped:tech:urls')) < 0) {
        await redisClient.expire('scraped:tech:urls', 7 * 24 * 3600);
      }
...
// Kickoff + interval (every 3 hours)
scrapeTechCrunch();
setInterval(scrapeTechCrunch, 3 * 60 * 60 * 1000);
