...
export default function SearchPage() {
  ...
  const { currUser ,lightMode,searchResults, setSearchResults} = useContext(myContext);
  ...
  useEffect(() => {
    async function fetchSearchResults() {
      ...
      const res=await axios.post(`${import.meta.env.VITE_API_URL}/api/article/recommendedarticles`, { embedding: embedding.data.embedding });
        ...
        console.log('✅ Final search results:', articles);
      } catch (err) {
        console.error('❌ Algolia fetch failed:', err);
        setSearchResults([]);
      }
    }
  ...
  if (trendingCategories === null || recentArticles === null || searchResults === null) {
    return (
  ...
  const mainArticles = Array.isArray(searchResults) ? searchResults : [];
    ...
    <div className={`min-h-screen transition-colors duration-300 ${lightMode ? 'bg-gray-50' : 'bg-gray-900'}`}>
      <div className="relative h-[30vh] sm:h-[35vh] md:h-[40vh] lg:h-[45vh] w-full overflow-hidden">
        ...
        <div className="relative z-10 flex items-end h-full px-4 sm:px-6 pb-8 sm:pb-12 md:pb-16">
          <div className="w-full max-w-7xl mx-auto">
            {/* Main Title */}
            <h1 className={`text-xl sm:text-2xl md:text-3xl lg:text-4xl font-bold mb-4 leading-tight transition-colors duration-300 ${lightMode ? 'text-gray-900' : 'text-white'
              }`}>
              Results for {query}
            </h1>
          ...
          <div className="flex flex-col xl:flex-row gap-6 lg:gap-8 relative">
            ...
            <div className="xl:w-80 space-y-6 sm:space-y-8 order-2 xl:order-1 xl:sticky xl:top-28 xl:self-start">
              ...
              <div className="space-y-4 sm:space-y-6">
                <h3 className={`text-lg font-bold transition-colors duration-300 ${lightMode ? 'text-gray-800' : 'text-gray-200'
                  }`}>Recent Posts</h3>
                    ...
                    <div key={post.id} className="flex gap-3 sm:gap-4 group cursor-pointer">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <h4
                          ...
                          className={`text-sm font-semibold leading-tight mb-1 sm:mb-2 cursor-pointer line-clamp-2 transition-all duration-300 group-hover:scale-[1.02] ${lightMode
                        ...
                        <p className={`text-xs font-medium transition-colors duration-300 ${lightMode ? 'text-gray-500' : 'text-gray-400'
                          }`}>
            ...
            <div className="flex-1 order-1 xl:order-2">
              <div className="mb-8">
                <div className={`
                    ...
                    `}>
                  <div className="w-2 h-2 bg-green-500 rounded-full animate-pulse"></div>
                  {currArticles?.length} results found
                </div>
              </div>
                      ...
                      <div className={`flex-1 p-6 flex flex-col justify-between relative overflow-hidden ${lightMode
                        ...
                        <div className="relative z-10">
                          ...
                          <p className={`text-sm line-clamp-3 leading-relaxed ${lightMode ? 'text-gray-700' : 'text-gray-300'
                            ...
                            {(article.description || article.summary)?.slice(0, 150) + " . . . . ."}
                          </p>
                        </div>
