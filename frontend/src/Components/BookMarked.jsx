...
export default function BookMarked() {
  ...
  const mainArticles = Array.isArray(articles) ? articles : [];
  ...
  const currArticles = mainArticles.slice(startIndex, startIndex + articlesPerPage);
  ...
  async function handleRemoveBookmark(articleId) {
    ...
    try {
      ...
      const updatedArticles = savedList.filter(function (article) {
        if (article._id !== articleId) {
          return article;
        }
      });

      setSavedList(updatedArticles);
 ...
 if (articles == null || trendingCategories == null || savedList == null) {
  return (
  ...
  <>
    ...
    <div className="relative h-[45vh] w-full overflow-hidden">
      ...
      >
        ...
        <div className="absolute inset-0 bg-gradient-to-r from-black/60 via-black/40 to-black/60 animate-pulse"></div>
        ...
        ></div>
        {/* Decorative Elements */}
        <div className="absolute top-10 right-10 w-32 h-32 bg-orange-500/20 rounded-full blur-3xl animate-bounce"></div>
        <div className="absolute bottom-20 left-20 w-24 h-24 bg-blue-500/20 rounded-full blur-2xl animate-pulse"></div>
      </div>
      ...
      <div className="relative z-10 flex items-end h-full px-4 sm:px-6 lg:px-8 pb-8 sm:pb-12 lg:pb-16">
        <div className="w-full max-w-7xl mx-auto">
          <div className="transform transition-all duration-1000 translate-y-0">
            <h1 className={`text-2xl sm:text-3xl md:text-4xl lg:text-6xl font-black ${lightMode ? 'text-gray-900' : 'text-white'} mb-4 leading-tight tracking-tight`}>
              <span className="bg-gradient-to-r from-orange-500 to-pink-500 bg-clip-text text-transparent">Saved</span>{' '}
              <span className="relative">
                Articles
                <div className="absolute -bottom-2 left-0 w-full h-1 bg-gradient-to-r from-orange-500 to-pink-500 rounded-full transform scale-x-0 animate-pulse group-hover:scale-x-100 transition-transform duration-700"></div>
              </span>
            </h1>
            <div className="w-20 h-1 bg-gradient-to-r from-orange-500 to-pink-500 rounded-full animate-pulse"></div>
          </div>
        ...
        <div className="flex flex-col xl:flex-row gap-6 lg:gap-8">
          ...
          <div className="xl:w-80 xl:sticky xl:top-28 xl:self-start order-2 xl:order-1">
            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-1 gap-6 lg:gap-8">
              ...
              <div className={`
                ...
                backdrop-blur-sm border shadow-2xl hover:shadow-3xl
                ${lightMode
                  ? 'bg-white/80 border-gray-200/50 shadow-gray-200/50'
                  : 'bg-gray-800/80 border-gray-700/50 shadow-black/20'
                }
          ...
          <div className="flex-1 order-1 xl:order-2">
            <div className="mb-8">
              ...
              `}>
                <div className="w-2 h-2 bg-green-500 rounded-full animate-pulse"></div>
                {mainArticles?.length} results found
              </div>
            </div>
              ...
              {currArticles.map((article, index) => (
                <div
                  ...
                  className={`
                    ...
                    transition-all duration-500 overflow-hidden transform hover:scale-[1.02]
                ...
                >
                  ...
                  <div className="relative flex flex-col sm:flex-row items-stretch">
                    ...
                    <div className="w-full sm:w-72 h-48 sm:h-auto relative group/image overflow-hidden sm:rounded-l-2xl">
                      ...
                      <div className="absolute top-4 right-4 z-10 p-2 rounded-full transition-all duration-300 cursor-pointer bg-black/60 hover:bg-red-500/80 backdrop-blur-sm border border-white/20 hover:border-red-300/50 transform hover:scale-110">
                        <X onClick={() => handleRemoveBookmark(article._id)} className="w-5 h-5 text-white" />
                      </div>
                    ...
                    <div className={`flex-1 p-6 flex flex-col justify-between relative overflow-hidden ${lightMode
                      ...
                      <div className="relative z-10">
                        {article.topic && (
                          <div
                            className={`inline-flex items-center gap-2 text-xs px-4 py-2 font-bold ${articleColors[index % articleColors.length]
                              } text-white rounded-full mb-4 shadow-lg backdrop-blur-sm border border-white/20 hover:scale-105 transition-transform duration-300`}
                          >
                            <div className="w-2 h-2 bg-white rounded-full animate-pulse"></div>
                            {topicMap[article.topic]?.icon || <Search fontSize="inherit" />}
